...
public class SetManager {
  ...
  private static void verifyMembership() {
    ...
    universalSet.forEach(element -> System.out.println(CYAN + "• " + element + " ∈ U" + RESET));
  }
  ...
  private static void classifySubsets() {
    ...
    subsets.forEach(subset -> {
      ...
      if (universalSet.contains(subset)) {
        isSubset = true;
      } else {
        isSubset = true;
      ...
      if (isSubset) {
        String type = subset.size() < universalSet.size() ? "⊂ U (Subconjunto propio)" : "⊆ U (Subconjunto impropio)";
  ...
  private static void executeOperation(List<Set<Object>> sets, int operation) {
    switch (operation) {
      case 1:
        ...
        break;
      case 2:
        performIntersection(sets);
        break;
      case 3:
        performDifference(sets);
        break;
      case 4:
        performSymmetricDifference(sets);
        break;
      case 5:
        performComplement(sets);
        break;
      case 6:
        performCartesianProduct(sets);
        break;
    }
  }
  ...
  // Se puede eliminar el método printOperationResult para el complemento o
  // dejarlo para otras operaciones
  ...
  private static void demonstrateAllLaws(Map<String, Set<Object>> namedSets) {
    ...
    namedSets.forEach((name, set) -> System.out.println(name + " = " + formatSet(set)));
  ...
  private static void cartesianProductHelper(List<Set<Object>> sets, int index, List<Object> current,
      Set<String> result) {
    if (index == sets.size()) {
